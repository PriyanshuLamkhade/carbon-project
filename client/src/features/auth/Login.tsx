...
const Login = () => {
    ...
    const responseGoogle = async(authResult:any) => {
        setLoading(true)
        try {
            const result = await axios.post(`${authService}/users/login`,{
    ...
    if(onboard){
        return(
            <RegistrationForm userData={tempUser}/>
        )
    }
  return (
    <div className="flex min-h-screen min-w-full  items-center justify-center px-4">
        <div className="w-full space-y-6  items-center ">
            ...
            hover:-translate-0.5"><FcGoogle size={20}/>{loading?"Signin in ...":"Continue with Google"}</button>
